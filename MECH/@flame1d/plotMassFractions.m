...
if nargin==3
    ...
    if isa(arg,'numeric')
        plot(obj.space,obj.massFractions(indx,:),marker);
        h = legend(makeNiceSpeciesNames(obj.speciesNames(indx)));        
    elseif isa(arg,'cell')&&isa(arg{1},'char')
        plot(obj.space,obj.getMassFractionsByName(arg),marker);
...
elseif nargin==2 % call with species specifies
    ...
    % species to be plottet...
    indx = arg;   
    % single massFractions Object
    plot(obj.space,obj.massFractions(obj.getSpeciesIndexByName(arg),:));
    h = legend(makeNiceSpeciesNames(indx)); 
    set(h,'Interpreter','TeX','Box', 'on','FontSize',20,'Location','Best');
    xlabel('t in sec','FontSize',20); 
    title('Mass Fractions','FontSize',20);
    ...
    [~,indx] = sort(m);
    indx = indx(end-5:end);
    plot(obj.space,obj.massFractions(indx,:));
    h = legend(makeNiceSpeciesNames(obj.speciesNames(indx)));
    set(h,'Interpreter','TeX','Box', 'on','FontSize',20,'Location','Best');
end
title('Mass Fractions','FontSize',20);
